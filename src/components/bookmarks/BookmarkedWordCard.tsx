...
interface ListInfo { id: number | string; name: string }
...
export const BookmarkedWordCard = ({ entry, isBookmarked, onToggleBookmark, onOpenListPicker, lists = [] }: BookmarkedWordCardProps) => (
  <div className="bg-white border border-gray-100 rounded-lg p-3 mb-3 shadow-sm hover:shadow-md transition-shadow">
    <div className="flex justify-between items-start mb-1">
      ...
      <BookmarkButton
          ...
          onClick={() => onToggleBookmark(entry.word, entry.furigana, entry.idseq)}
    ...
    {lists && lists.length > 0 && (
      <div className="mt-2 flex flex-wrap gap-2">
        {lists.filter(l => String(l.name).toLowerCase() !== 'default').map((l) => (
          <span key={String(l.id)} className="text-[11px] bg-indigo-50 text-indigo-700 px-2 py-0.5 rounded">{l.name}</span>
        ))}
      </div>
    )}
  </div>
