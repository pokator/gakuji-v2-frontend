  ...
  <div className="bg-amber-50 border border-amber-100 rounded-lg p-3 shadow-sm hover:shadow-md transition-all group">
    <div className="flex justify-between items-start">
      <div className="flex items-center gap-3 flex-1 min-w-0">
        <div className="text-3xl font-serif text-amber-900 bg-white rounded w-10 h-10 flex items-center justify-center border border-amber-100 flex-shrink-0">
          {bookmark.char}
        </div>
    ...
    <div className="mt-2 grid grid-cols-2 items-center text-xs gap-2">
      <div>
        {bookmark.radicals && bookmark.radicals.length > 0 && (
          <div className="text-[11px] text-amber-700/90">Radicals: {bookmark.radicals.join(' ')}</div>
        )}
      </div>
